  ...
  return (
    token && (
      <div className="bg-my-green p-5 md:mx-40 min-w-600 rounded-md">
        ...
        <button onClick={() => setSelectedArtists(select(90, 81))}>
          90 - 81
        </button>
        <button onClick={() => setSelectedArtists(select(80, 71))}>
          80 - 71
        </button>
        <button onClick={() => setSelectedArtists(select(70, 61))}>
          70 - 61
        </button>
        <button onClick={() => setSelectedArtists(select(60, 51))}>
          60 - 51
        </button>
        <button onClick={() => setSelectedArtists(select(50, 41))}>
          50 - 41
        </button>
        <button onClick={() => setSelectedArtists(select(40, 31))}>
          40 - 31
        </button>
        <button onClick={() => setSelectedArtists(select(30, 21))}>
          30 - 21
        </button>
        <button onClick={() => setSelectedArtists(select(20, 11))}>
          20 - 11
        </button>
        <button onClick={() => setSelectedArtists(select(10, 0))}>
          10 - 0
        </button>
        {selectedArtists && <p>Total artists: {selectedArtists.length}</p>}
        {selectedArtists &&
          selectedArtists.map((person, _) => {
